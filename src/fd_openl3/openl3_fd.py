...
def extract_embeddings(
    directory_path, channels, samplingrate, content_type, openl3_hop_size, batch_size=16
):
    """
    ...
    for i in range(0, len(wav_files), batch_size):
        ...
        for file in batch_files:
            ...
            if audio.shape[0] < sr:
                print(
                    "Audio shorter than 1 sec, openl3 will zero-pad it:",
                    file,
                    audio.shape,
                    sr,
                )
            ...
            audio = soxr.resample(
                audio, sr, samplingrate
            )  # mono/stereo <- mono/stereo, input sr, output sr
        ...
        # extracting stereo embeddings (dim=1024), since we concatenate L (dim=512) and R (dim=512) embeddings
            ...
            emb_r, _ = openl3.get_audio_embedding(
                batch_audio_r,
                batch_sr,
                model=model,
                verbose=False,
...
def extract_embeddings_nobatching(
    directory_path, channels, samplingrate, content_type, openl3_hop_size
):
    """
    ...
    for file in tqdm(wav_files):
        ...
        if channels == 2:
            ...
            emb, _ = openl3.get_audio_embedding(
                audio_l3, sr_l3, model=model, verbose=False, hop_size=openl3_hop_size
            )
            if audio.ndim == 1:
...
def openl3_fd(
    ...
    if load_ref_embeddings:
        ...
        if ref_path:
            ...
            if batching:
                ref_embeddings = extract_embeddings(
                    path,
                    channels,
                    samplingrate,
                    content_type,
                ...
                ref_embeddings = extract_embeddings_nobatching(
                    path, channels, samplingrate, content_type, openl3_hop_size
                )
    ...
    if batching:
        eval_embeddings = extract_embeddings(
            path,
            channels,
            samplingrate,
            content_type,
        ...
        eval_embeddings = extract_embeddings_nobatching(
            path, channels, samplingrate, content_type, openl3_hop_size
        )
