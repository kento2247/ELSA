...
class AudioCapEval:
    ...
    def test(self) -> dict:
        ...
        test_dataset = AudioCapDataset(data_dir=self.data_dir, split="test")
        ...
        if self.save_qualitative:
            ...
            df = pd.DataFrame({"score": all_scores, "pred": all_preds})
            df.to_csv(os.path.join(self.model_dir, "test_results.csv"), index=False)

        print(
            f"mse, pearson, spearman, kendall_tau = {mse_val}, {pearson_val}, {spearman_val}, {kendall_val}"
        )
