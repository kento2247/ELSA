...
class InversePatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.flatten:
            ...
            x = (
                x.transpose(1, 2).unflatten(2, self.grid_size).contiguous()
            )  # BNC -> BCHW
        x = self.proj(x)
...
class HTSAT_Decoder(nn.Module):
    ...
    def __init__(
        ...
        for i_layer in range(self.num_layers):
            ...
            self.skip.append(
                SkipTrans(
                    embed_dim=lan_embed_dim,
                    in_features=int(encoder_embed_dim * 2**i_layer),
                    out_features=int(self.embed_dim * 2**i_layer),
                ),
            )
        ...
        if not conv:
            ...
            transformer_encoder = nn.TransformerEncoder(
                encoder_layer, num_layers=n_masker_layer
            )
        ...
        self.film = SkipTrans(
            embed_dim=lan_embed_dim,
            in_features=encoder_embed_dim * 8,
    ...
    def forward(self, hidden_state, skip_features, embed):
        ...
        for i, (layer, f, skip) in enumerate(
            zip(self.layers, skip_features, self.skip)
        ):
            h = layer(h)[0]
        ...
        if self.phase:
            mask_r, mask_i = torch.chunk(
                self.phase_net(h).unsqueeze(1), chunks=2, dim=-1
            )
