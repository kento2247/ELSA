...
class UDiT(nn.Module):
    ...
    def __init__(
        ...
        self.pos_embed = PE_wrapper(
            dim=hidden_size, method=pos_method, length=pos_length
        )
        ...
        self.mid_block = DiTBlock(
            hidden_size, num_heads, mlp_ratio=mlp_ratio, use_checkpoint=use_checkpoint
        )
...
if __name__ == "__main__":
    with open(
        "/export/corpora7/HW/DPMTSE-main/src/config/DiffTSE_udit_conv_v_b_1000.yaml",
        "r",
    ) as fp:
        config = yaml.safe_load(fp)
    device = "cuda"

    model = UDiT(**config["diffwrap"]["UDiT"]).to(device)
