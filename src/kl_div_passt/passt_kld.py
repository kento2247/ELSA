...
def passt_kld(
    ...
    ):  # capturing all useless outputs from passt
        # load model
                ...
                if id not in no_ids:
                    try:
                        audio_path = os.path.join(
                            ref_path, str(id) + ref_files_extension
                        )
                        ref_p[id] = return_probabilities(
                            model, audio_path, collect=collect
                        )
                    except Exception as e:
                        print(
                            f"An unexpected error occurred with {id}: {e}\nIf you failed to download it you can add it to no_ids list."
                        )

            # store reference probabilities to load later on
            if not os.path.exists("load/passt_kld/"):
                os.makedirs("load/passt_kld/")
            save_ref_probabilities_path = (
                "load/passt_kld/"
                + ref_path.replace("/", "_")
                + "_collect"
                + str(collect)
                + "__reference_probabilities.pkl"
            )
    ...
    for id in tqdm(ids):
        if id not in no_ids:
            try:
                ...
                passt_kl += F.kl_div(
                    (ref_p[id] + 1e-6).log(), eval_p, reduction="sum", log_target=False
                )
                count += 1
            except Exception as e:
                print(
