...
- **高度な前処理パイプライン**: GPT-4o/Gemini/Qwen LLMによるテキスト分割、SAM-Audioによる音源分離

## インストール

```bash
git clone --recurse-submodules git@github.com:kento2247/TTAEval.git
...
| `--test_dataset_names` | `relate audiocap musiccap aishell7b` | テストするデータセット名 |
...
├── model.py         # TTAEvalModel（COGR + FIGR方式）
...
| `GPTTextParser` | GPT-4oによるキャプションの音声イベント分割 |
| `GeminiTextParser` | Gemini 2.5 Flashによるキャプションの音声イベント分割 |
...
├── parsed_texts/           # LLMによる分割テキスト（JSON）
...
- [OpenAI GPT-4o](https://platform.openai.com/) - テキスト分割用LLM
- [Gemini](https://ai.google.dev/) - テキスト分割用LLM
